...
struct QPacket wrQueueElements[SYS_CONSOLE_APPIO_WR_QUEUE_DEPTH];
struct QPacket rdQueueElements[SYS_CONSOLE_APPIO_RD_QUEUE_DEPTH];

struct QueueNode writeQueue = {0, 0, 0, wrQueueElements, SYS_CONSOLE_APPIO_WR_QUEUE_DEPTH};
struct QueueNode readQueue = {0, 0, 0, rdQueueElements, SYS_CONSOLE_APPIO_RD_QUEUE_DEPTH};
...
void Console_APPIO_Tasks (SYS_MODULE_OBJ object)
{
    ...
    switch(consAppIOData.state)
    {
        ...
        case CONSOLE_APPIO_STATE_WAIT_FOR_WRITE_COMPLETE:
            ...
            if(consAppIOData.writeCycleCount++ > pkt.sz * 200)
            {
