...
static const SYS_TMR_INIT       defaultTmrInit = 
{
    
    {SYS_MODULE_POWER_RUN_FULL},  // moduleInit
    DRV_TMR_INDEX_0,              // drvIndex
    1000,                         // tmrFreq    
};
        ...
        const SYS_MODULE_INIT * const init )
{
    ...
    while(true)
    {
        ...
        const SYS_TMR_INIT * sysTmrInit = ( const SYS_TMR_INIT * ) init;
        if(sysTmrInit == 0)
        {
            sysTmrInit = &defaultTmrInit;
        }
...
{
    return (sSysTmrObject.status == SYS_STATUS_READY) ? sSysTmrObject.sysTickCount : 0;
}


uint32_t SYS_TMR_TickCounterFrequencyGet ( void )
{
    return (sSysTmrObject.status == SYS_STATUS_READY) ? sSysTmrObject.sysTickFreq : 0;
}

uint32_t SYS_TMR_SystemCountFrequencyGet( void )
{
    return (sSysTmrObject.status == SYS_STATUS_READY) ? sSysTmrObject.driverFreq : 0;
}
...
uint64_t SYS_TMR_SystemCountGet( void )
{
    ...
    sll.low = sSysTmrObject.sysTickCount;
    sll.high = sSysTmrObject.sysTickCountHigh;

    currCount =  (sll.ull * sSysTmrObject.driverFreq ) / sSysTmrObject.sysTickFreq + DRV_TMR_CounterValue16BitGet ( sSysTmrObject.driverHandle );
    return currCount; 
}
