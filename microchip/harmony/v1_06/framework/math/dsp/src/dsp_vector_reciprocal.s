...
# void DSP_VectorRecip(_Q16 *outVector, _Q16 *inVector, int N)
#
# 692 (variable) clock cycles for a 4 element 32bit data vector
# 1162 (variable) clock cycles for a 8 element 32bit data vector
...
# Push stack
    addiu   $sp,$sp,-32
    swm     $s0-$s2, 0($sp)     # save registers
	sw      $ra,16($sp)
 
	sll     $a2, $a2, 2         # N * sizeof(int32) [inVector]
    addu    $s2, $a2, $a1       # end addr of input vector
    move    $s0, $a0            # hold output address
    move    $s1, $a1            # hold input address
    
loop:
    li      $a0, 0x00010000     # dividend in function is always 1
    jal     _LIBQ_Q16Div        # call div function
    lw      $a1, 0($s1)         # load inVector[i] to call function [delay slot]

    sw      $v0, 0($s0)         # store outVector[i]

    addiu   $s1, 4              # next inVector (32 bits)
    
    bne 	$s1, $s2, loop      # loop until we are done with input vector
    addiu   $s0, 4              # next outVector (32 bits)

#done
    lwm       $s0-$s2, 0($sp)   # restore registers
