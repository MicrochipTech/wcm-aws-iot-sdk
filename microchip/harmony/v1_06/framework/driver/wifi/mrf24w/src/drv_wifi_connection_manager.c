...
/* Function:
 ...
 Remarks:
    ...
    manager by setting #define WF_MODULE_CONNECTION_MANAGER == DRV_WIFI_DISABLED 
    in system_config.h
...
void DRV_WIFI_ConnectionStateGet(uint8_t *p_state)
{
    ...
    *p_state = msgData[0];        /* connection state */
    if ((*p_state == DRV_WIFI_CSTATE_CONNECTED_INFRASTRUCTURE) ||
        ...
        (*p_state == DRV_WIFI_CSTATE_CONNECTION_IN_PROGRESS && p_wifi_ConfigData->networkType == DRV_WIFI_NETWORK_TYPE_SOFT_AP))
    {
...
{
    if (TCPIP_MAC_RES_OK == WF_ConvPassphraseToKey(passphraseReady.keyLen,
                                                   passphraseReady.key,
