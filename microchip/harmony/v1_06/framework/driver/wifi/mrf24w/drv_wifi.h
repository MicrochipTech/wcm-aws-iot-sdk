...
/*  Management Message Error Codes
  ...
  Description
    ...
    function call results in a management message being sent, via SPI, to the MRF24W.
*/
typedef enum
{
    DRV_WIFI_SUCCESS                                              = 1,
...
/*
  ...
  Description:
    ...
    This enumeration identifies the choices the MRF24W TX mode. It is
...
/*
  ...
  Description:
    ...
    These defines identify various default Wi-Fi SoftAP settings that can be
...
/*
  ...
  Description:
    ...
    This enumeration identifies Wi-Fi device types. The only device
...
/*
  ...
  Description:
    ...
    This structure contains MRF24WG SoftAP context data. See
    DRV_WIFI_SoftAPContextSet().
*/
typedef struct
{
    /* Defines how to start the SoftAP network. See DRV_WIFI_SOFTAP_MODE. */
    /* Default is DRV_WIFI_SOFTAP_CONNECT_THEN_START. */
    //uint8_t  mode;

    /* When starting an SoftAP network, the SSID can be hidden in the beacons. */
    /* Set true to hide the SSID, else false.  Default is false. */
    bool hiddenSsid;
...
/*
  ...
  Description:
    ...
    Structure/union can be used in functions DRV_WIFI_SecurityWepSet,
    DRV_WIFI_SecurityWpaSet, and DRV_WIFI_SecurityWpsSet
*/
typedef union
{
...
/*******************************************************************************
  ...
  Remarks:
    ...
    Currently, this function performs no work, but that may change in the
    future. The Wi-Fi initialization takes place in a state machine called
    by MRF24W_MACInit().
 *******************************************************************************/
bool DRV_WIFI_Initialize(void* pNetIf);
...
/*******************************************************************************
  ...
  Summary:
    Deinitializes the MRF24WG Wi-Fi driver.
    <p><b>Implementation:</b> Dynamic</p>

  Description:
    This function deinitializes the MRF24WG driver. It also saves the Wi-Fi
...
/*******************************************************************************
  ...
  Parameters:
    p_context -  pointer to SoftAP context data; see definition for the
  ...
  Description:
    ...
    *p_state                                      Description
    ...
    DRV_WIFI_CSTATE_CONNECTED_INFRASTRUCTURE      Wi-Fi connected in
                                                   infrastructure mode
    DRV_WIFI_CSTATE_CONNECTED_ADHOC               Wi-Fi connected in adhoc
                                                   mode
...
/*******************************************************************************
  ...
  Description:
    This function allows the application to set the number of MRF24W
...
/*******************************************************************************
  ...
  Precondition:
    Wi-Fi initialization must be complete. Cannot be called when the MRF24W
...
/*******************************************************************************
  ...
  Parameters:
    p_factoryMaxTxPower -  pointer to where factory max power is written
                           (dbM)
...
/*******************************************************************************
  ...
  Description:
    ...
    to sleep and wake up at periodic intervals to check for data messages,
    which the Access Point will buffer. The listenInterval in the
    Connection Algorithm defines the sleep interval. By default, PS-Poll
    mode is disabled.

    When PS Poll is enabled, the Wi-Fi Host Driver will automatically force
    the MRF24W to wake up each time the Host sends TX data or a control
    message to the MRF24W. When the Host message transaction is complete
    the MRF24W driver will automatically re-enable PS Poll mode.
...
/*******************************************************************************
  ...
  Description:
    ...
    DRV_WIFI_PS_PS_POLL_DTIM_ENABLED    MRF24W in PS-Poll mode with DTIM
                                         enabled
    DRV_WIFI_PS_PS_POLL_DTIM_DISABLED   MRF24W in PS-Poll mode with DTIM
                                         disabled
...
/*******************************************************************************
  ...
  Description:
    Sets the RTS/CTS packet size threshold for when RTS/CTS frame will be
...
/*******************************************************************************
  ...
  Description:
    ...
    up they are ORed together - the MRF24W will receive and pass to the
  ...
  Precondition:
    Wi-Fi initialization must be complete.
    DRV_WIFI_SWMultiCastFilterEnable() must have been called previously.
...
/*******************************************************************************
  ...
  Summary:
    Sets a multicast address filter using one of the two hardware multicast
    filters.
    <p><b>Implementation:</b> Dynamic</p>

  Description:
    This function allows the application to configure up to two hardware
    Multicast Address Filters on the MRF24W. If two active multicast
    filters are set up they are ORed together - the MRF24W will receive and
    pass to the Host CPU received packets from either multicast address.
  ...
  Parameters:
    multicastFilterId -  DRV_WIFI_MULTICAST_FILTER_1 or
                         DRV_WIFI_MULTICAST_FILTER_2
...
/*******************************************************************************
  ...
  Description:
    Directs the MRF24W to initiate a scan operation. The Host Application
    will be notified that the scan results are ready when it receives the
    WF_EVENT_SCAN_RESULTS_READY event. The eventInfo field for this event
    will contain the number of scan results. Once the scan results are
    ready they can be retrieved with DRV_WIFI_ScanGetResult().
  ...
  Parameters:
    scanAll -  If false:
               ...
               If true -
                   * All scan results are retained (both Infrastructure and AdHoc networks).
...
/*******************************************************************************
  ...
  Precondition:
    Wi-Fi initialization must be complete. WF_EVENT_SCAN_RESULTS_READY
...
/*******************************************************************************
  ...
  Description:
    This function starts a gratuitous ARP response to be periodically
    transmitted.

  Precondition:
    Wi-Fi initialization must be complete. Connection process must be
    complete.
...
/*******************************************************************************
  ...
  Summary:
    Implements Wi-Fi driver deferred ISR.
    <p><b>Implementation:</b> Dynamic</p>

  Description:
    This function implements Wi-Fi driver deferred ISR.
  Precondition:
    TCP/IP stack should be initialized.
    DRV_WIFI_RTOS_TaskInit() must has been called previously.
...
/*******************************************************************************
  ...
  Summary:
    Takes semaphore in Wi-Fi deferred ISR.
    <p><b>Implementation:</b> Dynamic</p>

  Description:
    This function takes semaphore in Wi-Fi deferred ISR.
...
/*******************************************************************************
  ...
  Summary:
    Gives semaphore in Wi-Fi deferred ISR.
    <p><b>Implementation:</b> Dynamic</p>

  Description:
    This function gives semaphore in Wi-Fi Deferred ISR.
