...
static bool _DRV_TMR_ClockSourceSet(TMR_MODULE_ID timerId, DRV_TMR_CLK_SOURCES clockSource, bool update)
{
    ...
    {
        #if defined(PLIB_TMR_ExistsClockSource)
    ...
    else if(clockSource == DRV_TMR_CLKSOURCE_EXTERNAL_SYNCHRONOUS)
    {
            ...
            if ( PLIB_TMR_ExistsClockSource ( timerId ) )
            {
                if(update)
                {
                ...
                #if defined(PLIB_TMR_ExistsClockSourceSync)
                    ...
                    {
                        /* If clock source feature exists, but synchronization feature doesn't exist
                        for any specific timer module instance, then by default external clock is used
                        in synchronized mode, so do nothing */
                    }  
                #endif
                return true;
            }
    ...
    else if(clockSource == DRV_TMR_CLKSOURCE_EXTERNAL_ASYNCHRONOUS)
    {
        #if defined(PLIB_TMR_ExistsClockSourceSync)
            if ( PLIB_TMR_ExistsClockSourceSync ( timerId ) )
            {
...
{
#if defined(PLIB_TMR_ExistsPrescale)
...
/* Function:
  ...
  Description:
    ...
    processes the timer events in non interrupt-driven implementations
    (DRV_TMR_INTERRUPT_MODE == false).
