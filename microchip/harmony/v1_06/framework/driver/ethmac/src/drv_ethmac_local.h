...
// allows single/multiple clients access to the MAC
// This symbol should be defined if multiple clients access is allowed
// Under normal usage, the only client of the MAC driver is the TCP/IP stack
// However, for allowing other modules to interface directly with the MAC
// (to send/receive packets) this symbol could be defined 
//#define DRV_ETHMAC_MULTI_CLIENT_ACCESS
...
typedef uint16_t (*DRV_ETHMAC_HW_REG_FUNC)(int index);
