...
static uint16_t     _DRV_ETHMAC_GetFrmTxOk(int index);
static uint16_t     _DRV_ETHMAC_GetFrmRxOk(int index);
static uint16_t     _DRV_ETHMAC_GetFrmPktCount(int index);
static uint16_t     _DRV_ETHMAC_GetFrmRxOvflow(int index);
static uint16_t     _DRV_ETHMAC_GetFrmFcsError(int index);
static uint16_t     _DRV_ETHMAC_GetAlignError(int index);
static uint16_t     _DRV_ETHMAC_GetSCollisionCount(int index);
static uint16_t     _DRV_ETHMAC_GetMCollisionCount(int index);
...
SYS_MODULE_OBJ DRV_ETHMAC_PIC32MACInitialize(const SYS_MODULE_INDEX index, const SYS_MODULE_INIT * const init)
{
	...
	while(1)
	{
        ...
        PLIB_ETH_MACSetMaxFrame(macIx, TCPIP_EMAC_RX_MAX_FRAME);


        sPhyInit.moduleInit.value = 0;
        sPhyInit.ethphyId = phyIx;
        ...
        PLIB_ETH_RxFiltersClr(macIx, ETH_FILT_ALL_FILTERS);
        PLIB_ETH_RxFiltersSet(macIx, ETH_FILT_CRC_ERR_REJECT | ETH_FILT_RUNT_REJECT | ETH_FILT_ME_UCAST_ACCEPT | ETH_FILT_MCAST_ACCEPT | ETH_FILT_BCAST_ACCEPT);
        ...
        {   // use the supplied address
            PLIB_ETH_MACSetAddress(macIx, alignMacAddress.addr);
        }
...
#if defined (DRV_ETHMAC_MULTI_CLIENT_ACCESS)
            else
...
TCPIP_MAC_RES DRV_ETHMAC_PIC32MACRxFilterHashTableEntrySet(DRV_HANDLE hMac, TCPIP_MAC_ADDR* DestMACAddr)
{
      ...
      int index = ((DRV_ETHMAC_INSTANCE_DCPT*)hMac)->mData._macIx;
...
union
{
    ...
    crc32_val.Val = 0xFFFFFFFF;
      ...
      if( DestMACAddr == 0 || memcmp(DestMACAddr->v, nullMACAddr, sizeof(nullMACAddr))==0 )
      {
          ...
          PLIB_ETH_RxFiltersClr(index, ETH_FILT_HTBL_ACCEPT);
          PLIB_ETH_RxFiltersHTSet(index, 0ull);
          return TCPIP_MAC_RES_OK;
      }
      ...
      *pHTSet = 1 << hVal;
      ...
      PLIB_ETH_RxFiltersSet(index, ETH_FILT_HTBL_ACCEPT);
    ...
    {   // the rxBuffSize is viewed as total packet size, including the ETH frame
        ...
        *pHdrSpace = (uint8_t*)pRSeg->segLoad - (uint8_t*)pRxPkt;
        ...
        if(setRxSize)
        {
            ...
            PLIB_ETH_RxSetBufferSize(pMacD->mData._macIx, pMacD->mData.macConfig.rxBuffSize);
...
TCPIP_MAC_RES DRV_ETHMAC_PIC32MACRegisterStatisticsGet(DRV_HANDLE hMac, TCPIP_MAC_STATISTICS_REG_ENTRY* pRegEntries, int nEntries, int* pHwEntries)
{
    ...
    {   // fill data
        ...
        for(ix = 0; ix < regLim; ix++, pRegEntries++, pHwRegDcpt++)
        {
            ...
            pRegEntries->registerValue = (*pHwRegDcpt->regFunc)(pMacD->mData._macIx); 
        }
    }
...
TCPIP_MAC_RES DRV_ETHMAC_PIC32MACParametersGet(DRV_HANDLE hMac, TCPIP_MAC_PARAMETERS* pMacParams)
{
    ...
    { 
        if(pMacParams)
        {
            PLIB_ETH_MACGetAddress(pMacD->mData._macIx, pMacParams->ifPhyAddress.v); 
...
bool DRV_ETHMAC_PIC32MACEventMaskSet(DRV_HANDLE hMac, TCPIP_MAC_EVENT macEvMask, bool enable)
{
    ...
    if(enable)
    {
        ...
        if(pDcpt->_TcpEnabledEvents != 0)
        {
            ...
            PLIB_ETH_EventsClr(index, ethSetEvents );   // clear the old pending ones
            PLIB_ETH_EventsEnableSet(index, ethSetEvents );                 // enable the new un-ack ones!
    ...
    {   // disable some events
        ...
        PLIB_ETH_EventsEnableClr(index, ethClrEvents);   // no longer enabled
        PLIB_ETH_EventsClr(index, ethClrEvents);         // clear the pending ones
...
bool DRV_ETHMAC_PIC32MACEventAcknowledge(DRV_HANDLE hMac, TCPIP_MAC_EVENT tcpAckEv)
{
    ...
    {   // already have some active     
        ...
        PLIB_ETH_EventsClr(index, ethAckEv);                 // clear the old pending ones
        PLIB_ETH_EventsEnableSet(index, ethAckEv);           // re-enable the ack ones
...
void DRV_ETHMAC_Tasks_ISR( SYS_MODULE_OBJ macIndex )
{
    ...
    currEthEvents = PLIB_ETH_EventsGet(index);
    ...
    if(currGroupEvents)
    {
        ...
        if(isTxInterrupt)
        {
            PLIB_ETH_EventsEnableClr(index, currGroupEvents & ~ETH_EV_TXDONE);   // don't disable TX_DONE
            PLIB_ETH_EventsClr(index, currGroupEvents | ETH_EV_TXDONE);          // always ack TX_DONE
        }
        else
#endif  // defined(ETH_PIC32_INT_MAC_ISR_TX)
        {
            PLIB_ETH_EventsEnableClr(index, currGroupEvents);         // these will get reported; disable them until ack is received back
            PLIB_ETH_EventsClr(index, currGroupEvents);               // acknowledge the ETHC
        }
...
static uint16_t _DRV_ETHMAC_GetFrmTxOk(int index)
{
    return PLIB_ETH_FramesTxdOkCountGet(index);
}

static uint16_t _DRV_ETHMAC_GetFrmRxOk(int index)
{
    return PLIB_ETH_FramesRxdOkCountGet(index);
}

static uint16_t _DRV_ETHMAC_GetFrmPktCount(int index)
{
    return PLIB_ETH_RxPacketCountGet(index);
}

static uint16_t _DRV_ETHMAC_GetFrmRxOvflow(int index)
{
    return PLIB_ETH_RxOverflowCountGet(index);
}

static uint16_t _DRV_ETHMAC_GetFrmFcsError(int index)
{
    return PLIB_ETH_FCSErrorCountGet(index);
}

static uint16_t _DRV_ETHMAC_GetAlignError(int index)
{
    return PLIB_ETH_AlignErrorCountGet(index);
}

static uint16_t _DRV_ETHMAC_GetSCollisionCount(int index)
{
    return PLIB_ETH_SingleCollisionCountGet(index);
}

static uint16_t _DRV_ETHMAC_GetMCollisionCount(int index)
{
    return PLIB_ETH_MultipleCollisionCountGet(index);
}
