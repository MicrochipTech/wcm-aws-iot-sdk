...
#if (DRV_USART_BYTE_MODEL_SUPPORT == true)

    /* If byte model support is enabled, then interrupts should be disabled
       and tasks routines should not run. */
...
*/
    ...
    #define _DRV_USART_TRANSMIT_BUFFER_QUEUE_TASKS(x)
    #define _DRV_USART_RECEIVE_BUFFER_QUEUE_TASKS(x)
