...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _ADC_MUX_CHANNEL0_NEGATIVE_INPUT_VREG(index)
    _ADC_MUXA_NEGATIVE_INPUT_VREG(index)
    _ADC_MUXB_NEGATIVE_INPUT_VREG(index)

  MASKs:
    _ADC_MUX_CHANNEL0_NEGATIVE_INPUT_MASK(index)
    _ADC_MUXA_NEGATIVE_INPUT_MASK(index)
    _ADC_MUXB_NEGATIVE_INPUT_MASK(index)

  POSs:
    _ADC_MUX_CHANNEL0_NEGATIVE_INPUT_POS(index)
    _ADC_MUXA_NEGATIVE_INPUT_POS(index)
    _ADC_MUXB_NEGATIVE_INPUT_POS(index)

  LENs:
    _ADC_MUX_CHANNEL0_NEGATIVE_INPUT_LEN(index)
    _ADC_MUXA_NEGATIVE_INPUT_LEN(index)
    _ADC_MUXB_NEGATIVE_INPUT_LEN(index)
...
PLIB_TEMPLATE void ADC_MuxChannel0InputNegativeSelect_Default( ADC_MODULE_ID index , ADC_MUX muxType , ADC_INPUTS_NEGATIVE input )
{
    ...
    {
		_SFR_BIT_WRITE(_ADC_MUXB_NEGATIVE_INPUT_VREG(index),
		               _ADC_MUXB_NEGATIVE_INPUT_POS(index) ,
                       input);
	}
    else
    {
		_SFR_BIT_WRITE(_ADC_MUXA_NEGATIVE_INPUT_VREG(index),
		               _ADC_MUXA_NEGATIVE_INPUT_POS(index) ,
                       input);
	}
}
