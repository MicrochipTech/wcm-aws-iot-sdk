...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _ADC_CONVERSION_CLOCK_VREG(index)

  MASKs:
    _ADC_CONVERSION_CLOCK_MASK(index)

  POSs:
    _ADC_CONVERSION_CLOCK_POS(index)

  LENs:
    _ADC_CONVERSION_CLOCK_LEN(index)
...
PLIB_TEMPLATE void ADC_ConversionClockSet_Default( ADC_MODULE_ID index , uint32_t baseFrequency , ADC_CONVERSION_CLOCK value )
{
    ...
    _SFR_FIELD_WRITE(_ADC_CONVERSION_CLOCK_VREG(index),
						_ADC_CONVERSION_CLOCK_MASK(index),
						_ADC_CONVERSION_CLOCK_POS(index) ,
						regValue );
}
...
PLIB_TEMPLATE uint32_t ADC_ConversionClockGet_Default( ADC_MODULE_ID index , uint32_t baseFrequency )
{
	...
	regValue = _SFR_FIELD_READ(_ADC_CONVERSION_CLOCK_VREG(index),
							_ADC_CONVERSION_CLOCK_MASK(index),
							_ADC_CONVERSION_CLOCK_POS(index));
