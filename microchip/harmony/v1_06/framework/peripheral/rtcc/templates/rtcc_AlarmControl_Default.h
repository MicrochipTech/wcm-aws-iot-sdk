...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _RTCC_ALARM_CONTROL_VREG(index)

  MASKs: 
    _RTCC_ALARM_CONTROL_MASK(index)

  POSs: 
    _RTCC_ALARM_CONTROL_POS(index)

  LENs: 
    _RTCC_ALARM_CONTROL_LEN(index)
...
{
    /* TODO: Need to check Enable bit or ALRMSYNC bit first before changing? */
	_SFR_BIT_SET( _RTCC_ALARM_CONTROL_VREG( index ),
	              _RTCC_ALARM_CONTROL_POS( index ) );
}
...
{
    /* TODO: Need to check Enable bit or ALRMSYNC bit first before changing? */
	_SFR_BIT_CLEAR( _RTCC_ALARM_CONTROL_VREG( index ),
	                _RTCC_ALARM_CONTROL_POS( index ) );
}
