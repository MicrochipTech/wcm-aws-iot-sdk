...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _RTCC_CALIBRATION_VREG(index)

  MASKs: 
    _RTCC_CALIBRATION_MASK(index)

  POSs: 
    _RTCC_CALIBRATION_POS(index)

  LENs: 
    _RTCC_CALIBRATION_LEN(index)
...
{
    return _SFR_FIELD_READ( _RTCC_CALIBRATION_VREG( index ),
							_RTCC_CALIBRATION_MASK( index ),
	                        _RTCC_CALIBRATION_POS( index ) );
}
...
{
    _SFR_FIELD_WRITE( _RTCC_CALIBRATION_VREG( index ),
	                  _RTCC_CALIBRATION_MASK( index ),
	                  _RTCC_CALIBRATION_POS( index ),
					  calibrationbits );
}
