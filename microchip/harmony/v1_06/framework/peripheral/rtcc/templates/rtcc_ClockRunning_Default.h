...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _RTCC_CLOCK_RUNNING_VREG(index)

  MASKs: 
    _RTCC_CLOCK_RUNNING_MASK(index)

  POSs: 
    _RTCC_CLOCK_RUNNING_POS(index)

  LENs: 
    _RTCC_CLOCK_RUNNING_LEN(index)
...
{
    return _SFR_BIT_READ( _RTCC_CLOCK_RUNNING_VREG( index ),
	                      _RTCC_CLOCK_RUNNING_POS( index ) );
}
