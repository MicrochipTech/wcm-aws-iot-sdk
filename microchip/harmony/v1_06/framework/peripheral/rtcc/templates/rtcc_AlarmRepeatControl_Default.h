...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _RTCC_ALARM_REPEAT_COUNTER_VREG(index)

  MASKs: 
    _RTCC_ALARM_REPEAT_COUNTER_MASK(index)

  POSs: 
    _RTCC_ALARM_REPEAT_COUNTER_POS(index)

  LENs: 
    _RTCC_ALARM_REPEAT_COUNTER_LEN(index)
...
{
    _SFR_FIELD_WRITE( _RTCC_ALARM_REPEAT_COUNTER_VREG( index ),
	                  _RTCC_ALARM_REPEAT_COUNTER_MASK( index ),
					  _RTCC_ALARM_REPEAT_COUNTER_POS( index ),
					  data );
}
...
{
    return _SFR_FIELD_READ( _RTCC_ALARM_REPEAT_COUNTER_VREG( index ),
	                        _RTCC_ALARM_REPEAT_COUNTER_MASK( index ),
							_RTCC_ALARM_REPEAT_COUNTER_POS( index ) );
}
