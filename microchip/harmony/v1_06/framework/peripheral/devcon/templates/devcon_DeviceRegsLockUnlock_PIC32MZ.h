...
PLIB_TEMPLATE void DEVCON_DeviceRegistersLock_PIC32MZ( DEVCON_MODULE_ID index , DEVCON_REGISTER_SET registersToLock )
{
    ...
    {
        _SFR_BIT_SET(_DEVCON_PPS_LOCK_VREG(index),_DEVCON_PPS_LOCK_POS(index));
    }

    if ( 0 != (registersToLock & DEVCON_PMD_REGISTERS ))
    {
        _SFR_BIT_SET(_DEVCON_PMD_LOCK_VREG(index),_DEVCON_PMD_LOCK_POS(index));
    }

    if ( 0 != (registersToLock & DEVCON_PERMISSION_GROUP_REGISTERS ))
    {
        _SFR_BIT_SET(_DEVCON_PERMISSION_GROUP_LOCK_VREG(index),_DEVCON_PERMISSION_GROUP_LOCK_POS(index));
    }
...
PLIB_TEMPLATE void DEVCON_DeviceRegistersUnlock_PIC32MZ( DEVCON_MODULE_ID index , DEVCON_REGISTER_SET registersToLock )
{
    if ( 0 != (registersToLock & DEVCON_PPS_REGISTERS ))
    {
        _SFR_BIT_CLEAR(_DEVCON_PPS_LOCK_VREG(index),_DEVCON_PPS_LOCK_POS(index));
    }

    if ( 0 != (registersToLock & DEVCON_PMD_REGISTERS ))
    {
        _SFR_BIT_CLEAR(_DEVCON_PMD_LOCK_VREG(index),_DEVCON_PMD_LOCK_POS(index));
    }

    if ( 0 != (registersToLock & DEVCON_PERMISSION_GROUP_REGISTERS ))
    {
        _SFR_BIT_CLEAR(_DEVCON_PERMISSION_GROUP_LOCK_VREG(index),_DEVCON_PERMISSION_GROUP_LOCK_POS(index));
    }
