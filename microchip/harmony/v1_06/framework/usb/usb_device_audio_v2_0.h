...
/* USB Device Audio v2.0 Function Driver Index
  ...
  Description:
    This uniquely identifies a Audio v2.0 Function Driver instance.
 ...
 *  USB_DEVICE_Audio_V2_Write() , USB_DEVICE_Audio_V2_Read(), functions when the
...
#define USB_DEVICE_AUDIO_V2_TRANSFER_HANDLE_INVALID/*DOM-IGNORE-END*/((USB_DEVICE_AUDIO_V2_TRANSFER_HANDLE)(-1))/*DOM-IGNORE-END*/
...
/**************************************************************************
  ...
  Parameters:
    ...
    data  -           pointer to the data buffer contains the data to be written. 
                      ...
                      coherent memory and should be aligned a 4 byte boundary.
  ...
  Example:
    ...
    unit8_t txBuffer[192]; // Use this attribute for PIC32MZ __attribute__((coherent, aligned(4)))
  ...
  Remarks:
    ...
    placed in coherent memory and aligned at a 4 byte boundary.  This can be
    done by declaring the buffer using the  __attribute__((coherent,
    aligned(4))) attribute. An example is shown here                                                                   

    <code>
    uint8_t data[256] __attribute__((coherent, aligned(4)));
    </code>                                                                  
*/
...
/***************************************************************************
  ...
  Example:
    ...
    USB_DEVICE_AUDIO_V2_TRANSFER_HANDLE transferHandle;
    unit8_t interfaceNumber;
    // Use this attribute for PIC32MZ __attribute__((coherent, aligned(4)))
  ...
  Remarks:
    ...
    coherent memory and aligned at a 4 byte boundary.  This can be done by
    declaring the buffer using the  __attribute__((coherent, aligned(4)))
...
/* Function:
  ...
  Description:
    ...
    USB_DEVICE_AUDIO_V2_Write() function or the
    USB_DEVICE_AUDIO_V2_SerialStateNotificationSend() functions. If a transfer is
    still in the queue and its processing has not started, then the transfer is
    canceled completely. A transfer that is in progress may or may not get
...
/*DOM-IGNORE-BEGIN*/
extern const USB_DEVICE_FUNCTION_DRIVER audioV2FunctionDriver;/*DOM-IGNORE-END*/
