...
)
{
            ...
            (deviceObj->configDescriptorInfo.configurationNumber != USB_HOST_CONFIGURATION_NUMBER_INVALID))
    {
            ...
            else
            {
                ...
                if((deviceObj->deviceClientDriver != NULL) && (!interfaceInfo->wasTriedWithDeviceDriver))
                {
                    ...
                    else if(interfaceInfo->interfaceDescriptor != NULL)
                    {
                         ...
                         * will try rematching the interface. */
        ...
        /* We have to keep check if the all the interfaces and the device are
         * owned. If we reach a point where none are owned and the search has
         * reachd the end of TPL, then we move the device to an error state.
                ...
                (_USB_HOST_NoInterfacesOwned(deviceObj)))
        {
            ...
            SYS_DEBUG_MESSAGE(SYS_ERROR_INFO, "USB Host: Device not matched");
    ...
    if(deviceObj->deviceState == USB_HOST_DEVICE_STATE_READY)
    {
        switch(deviceObj->configurationState)
        {
            case USB_HOST_DEVICE_CONFIG_STATE_READY_FOR_CONFIG:
                ...
                /* We dont have to do anything here. The state indicates that
            ...
            case USB_HOST_DEVICE_CONFIG_STATE_START:
                 ...
                 * member of deviceObj contains the configuration number to be
                 * set */
                            ...
                            &(deviceObj->controlTransferObj.controlIRP)))
                {
                    ...
                    SYS_DEBUG_MESSAGE(SYS_ERROR_INFO, "USB Host: Could not send IRP");
                    deviceObj->deviceState = USB_HOST_DEVICE_STATE_ERROR;
                    deviceObj->hcdInterface->hostPipeClose(deviceObj->controlPipeHandle);
                    if(gUSBHostObj.hostEventHandler != NULL)
                    {
                        /* Send an event to the application */
                        gUSBHostObj.hostEventHandler( USB_HOST_EVENT_DEVICE_UNSUPPORTED ,
                                NULL, gUSBHostObj.context );
                    }
                }
            ...
            case USB_HOST_DEVICE_CONFIG_STATE_CONFIG_DESCRIPTOR_GET:
                 ...
                 * 7 addtionanal bytes to store the end of configuration
                ...
                if(deviceObj->configDescriptorInfo.configurationDescriptor == NULL)
                {
                    ...
                    if(gUSBHostObj.hostEventHandler != NULL)
                    {
                        SYS_DEBUG_MESSAGE(SYS_ERROR_INFO, "USB Host: Out of Memory\n");
                        gUSBHostObj.hostEventHandler( USB_HOST_EVENT_DEVICE_UNSUPPORTED , NULL, gUSBHostObj.context );
                    }
                }
                else
            ...
            case USB_HOST_DEVICE_CONFIG_STATE_WAIT_FOR_CONFIGURATION_SET:
                ...
                if (deviceObj->controlTransferObj.controlIRP.status == USB_HOST_IRP_STATUS_COMPLETED) 
                {
                    ...
                    else
                    {
                        ...
                        deviceObj->configDescriptorInfo.configurationNumber = 
                                deviceObj->configDescriptorInfo.configurationDescriptor->bConfigurationValue;
    ...
    if(deviceObj->deviceState == USB_HOST_DEVICE_STATE_READY)
    {
        ...
        if((deviceObj->deviceClientDriver == NULL) && (!(deviceObj->tplEntryTried >= gUSBHostObj.nTPLEntries)))
        {
            ...
            if(deviceObj->deviceClientDriver != NULL)
            {
                ...
                deviceObj->deviceClientDriver->deviceAssign(deviceObj->deviceClientHandle, 
                        deviceObj->deviceIdentifier, &(deviceObj->deviceDescriptor));
            }
        ...
        if(deviceObj->deviceClientDriver == NULL)
        {
            ...
            if(OSAL_MUTEX_Lock(&(gUSBHostObj.mutexControlTransferObj), OSAL_WAIT_FOREVER) == OSAL_RESULT_TRUE)
            {
                        ...
                        && (deviceObj->configDescriptorInfo.configurationNumber == USB_HOST_CONFIGURATION_NUMBER_INVALID))
                {
                     ...
                     * and has atleast one configuration. Check if the control
    ...
    else
    {
        ...
        *transferHandle = USB_HOST_TRANSFER_HANDLE_INVALID;
        ...
        else
        {
            ...
            /* Get a mutex lock as this is a global resource */
            if(OSAL_MUTEX_Lock(&(gUSBHostObj.mutexControlTransferObj), OSAL_WAIT_FOREVER) == OSAL_RESULT_TRUE)
            {
                if(!deviceObj->controlTransferObj.inUse)
                {
            ...
            {
                /* The mutex could not be obtained */
                result = USB_HOST_RESULT_REQUEST_BUSY;
            }
    ...
    else
    {
        ...
        else
        {
            ...
            while(search < ((uint8_t *)(configurationDescriptor) + configDescriptorSize))
            {
                ...
                {
                    /* This means we have found all the interfaces. Dont bother
                     * searching any more. */
                    break;
                }
            }
    ...
    else
    {
        switch (deviceObj->deviceState)
        {
            case USB_HOST_DEVICE_STATE_WAITING_FOR_ENUMERATION:
                ...
                /* If another device is enumerating on the bus, then we dont do
            ...
            case USB_HOST_DEVICE_STATE_WAITING_FOR_RESET_COMPLETE:
                ...
                if(deviceObj->hubInterface->hubPortResetIsComplete( deviceObj->hubHandle ,deviceObj->devicePort ))
                {
                    ...
                    busObj->busOperationsTimerHandle = SYS_TMR_DelayMS(100);
                }
            ...
            case USB_HOST_DEVICE_STATE_WAITING_FOR_RESET_SETTLING_DELAY_COMPLETE:
                ...
                if(SYS_TMR_DelayStatusGet( busObj->busOperationsTimerHandle))
                {
                    ...
                    if(DRV_USB_HOST_PIPE_HANDLE_INVALID == deviceObj->controlPipeHandle)
                    {
                        ...
                        SYS_DEBUG_MESSAGE(SYS_ERROR_INFO, "USB Host: Could not open pipe");
                        deviceObj->deviceState = USB_HOST_DEVICE_STATE_ERROR;
                        if(gUSBHostObj.hostEventHandler != NULL)
                        {
                            /* Send an event to the application */
            ...
            case USB_HOST_DEVICE_STATE_WATING_FOR_SET_ADDRESS_COMPLETE:
                ...
                if ( deviceObj->controlTransferObj.controlIRP.status == USB_HOST_IRP_STATUS_COMPLETED )
                {
                    ...
                    /* Close the current control pipe. This one is to device
                     * address 0 */
                    deviceObj->hcdInterface->hostPipeClose(deviceObj->controlPipeHandle); 
                ...
                else
                {
                    if ( deviceObj->controlTransferObj.controlIRP.status < USB_HOST_IRP_STATUS_COMPLETED )
                    {
                        /* The Set Address Request failed. We should either retry
                         * or place the device in error state. In any case the 
                         * the control pipe should be closed. */
            ...
            case USB_HOST_DEVICE_STATE_GET_DEVICE_DESCRIPTOR_FULL:
                ...
                _USB_HOST_FillSetupPacket(
                        ...
                        deviceObj->deviceDescriptor.bLength ) ;
            ...
            case USB_HOST_DEVICE_STATE_GET_CONFIGURATION_DESCRIPTOR_FULL:
                ...
                /* Here are will try to allocate memory for the full
            ...
            case USB_HOST_DEVICE_STATE_READY:
                 ...
                 * The host layer will now try to matcht the device to the
...
// *****************************************************************************
/* Function:
    void _USB_HOST_UpdateDeviceTask( USB_HOST_BUS_OBJ *busObj )
...
void _USB_HOST_UpdateDeviceTask( USB_HOST_BUS_OBJ *busObj )
{
    ...
    /* The first device on the bus is the root hub. We dont maintain the state
    ...
    while(deviceObj != NULL)
    {
         ...
         * state */
        _USB_HOST_MakeDeviceReady(deviceObj, busObj);

        /* If the device is not owned then find a driver that can own it */
        _USB_HOST_UpdateDeviceOwnership (deviceObj, busObj);
        
        /* If the configuration needs to be set, then set the configuration */
        _USB_HOST_UpdateConfigurationState(deviceObj, busObj);
        
        /* If the device is configured, update the state of the interfaces */
        _USB_HOST_UpdateInterfaceStatus(deviceObj, busObj);
    ...
    switch ( controlTransferObj->requestType )
    {
        ...
        case USB_HOST_CONTROL_REQUEST_TYPE_STRING_DESCRIPTOR:
            ...
            if(controlTransferObj->callback != NULL)
            {
                ...
                if(result == USB_HOST_RESULT_SUCCESS)
                {
                    ((USB_HOST_STRING_REQUEST_COMPLETE_CALLBACK)(controlTransferObj->callback))((USB_HOST_REQUEST_HANDLE)controlTransferObj, irp->size, controlTransferObj->context);
...
)
{
    ...
    /* Typecase the initialization data parameter
     * and check if it is NULL. The host layer 
     * cannot be initialized if the initialization
    ...
    /* Return pointer to the host layer object as the 
     * system module object. */
    return(( SYS_MODULE_OBJ )hostObj);
}
...
/* Function:
  ...
  Description:
    ...
    The  USB_HOST_DeviceGetFirst() function should have been called atleast once
...
USB_HOST_RESULT USB_HOST_DeviceGetNext (USB_HOST_DEVICE_INFO * deviceInfo)
{
    ...
    else
    {
        ...
        else
        {
             ...
             * that is in the device list. If they dont match then this means
...
)
{
    ...
    USB_HOST_DEVICE_OBJ  * deviceObj;
    OSAL_CRITSECT_DATA_TYPE criticalSectData;
    ...
    /* We have to enter a critical section here so that the process of device
     * object allocation does not get interrupted */

    criticalSectData = OSAL_CRIT_Enter(OSAL_CRIT_TYPE_HIGH);
    ...
    /* We can end the critical section as we have completed the search */
    OSAL_CRIT_Leave(OSAL_CRIT_TYPE_HIGH, criticalSectData);
    ...
    if(newDeviceObj != NULL)
    {
        ...
        /* Allocate a Plug N Play identifer. This identifier will be unique
...
/* Function:
  ...
  Summary:
    Denumerates an attached device.

  Description:
    This function denumerates an attached device. This function is called by the
...
void USB_HOST_DeviceDenumerate( USB_HOST_DEVICE_OBJ_HANDLE deviceObjHandle )
{
    ...
    if(deviceObjHandle != USB_HOST_DEVICE_OBJ_HANDLE_INVALID)
    {
         ...
         * mutltple entries on the same device. This may occur especially in the
        ...
        if(deviceObj->inUse)
        {
            ...
            /* Free any allocated memory. Do a NULL check just in case we catch this
             * when no configuration is loaded in the rawBuffer */
            if(deviceObj->configDescriptorInfo.configurationDescriptor != NULL)
            {
                USB_HOST_FREE(deviceObj->configDescriptorInfo.configurationDescriptor);
            }

            /* The holdingConfigurationDescriptor member will hold a configuration 
             * descriptor for the client. We need to make sure that this memory if
             * not already freed is freed up */
            if(deviceObj->configDescriptorInfo.configurationDescriptor != deviceObj->holdingConfigurationDescriptor)
            {
                if(deviceObj->holdingConfigurationDescriptor != NULL)
                {
                    USB_HOST_FREE(deviceObj->holdingConfigurationDescriptor);
    ...
    {
        /* The device object handle is not valid. We dont have to do anything */
    }
}
    ...
    for ( hcCount = 0 ; hcCount < USB_HOST_CONTROLLERS_NUMBER ; hcCount++ )
    {
        ...
        switch ( busObj->state)
        {
            ...
            case USB_HOST_BUS_STATE_ENABLING:
                ...
                busObj->hcdHandle = busObj->hcdInterface->open(  busObj->hcdIndex, 
            ...
            case USB_HOST_BUS_STATE_ENABLED:
                ...
                _USB_HOST_UpdateDeviceTask( busObj );
                break;
...
/* Function:
  ...
  This function queries the specified configuration for the specified endpoint
    ...
    can be called repetively to continue the search till the end of the
...
)
{
    ...
    else
    {
         ...
         * we dont have anything to search. */
        ...
        else
        {
            ...
            for(iterator = 0; iterator < bNumEndPoints; iterator++) 
            {
                if(search < lastLocation)
                {
                    ...
                    {
                        /* C
                         * {heck if the endpoint address matches the address */
                    ...
                    else
                    {
                        ...
                        {
                            /* This means all the criterias matched. We can end the
                             * search. */
...
/* Function:
  ...
  Description:
    ...
    pointer will point to the standard interace descriptor and class specific
    ...
    function can be called repetively to continue the search till the end of the
    ...
    else
    {
        ...
        *transferHandle = USB_HOST_TRANSFER_HANDLE_INVALID;
        ...
        else
        {
             ...
             * another thread. We will use a mutex to protect the transfer
             * object pool. But the mutex should be take only if the host if
             * this function is not being called in an interrupt context.*/

            if(!hostObj->isInInterruptContext)
            {
                /* We can take Mutex only if we are not in interrupt context */
             ...
             * mutex was taken, or if the mutex failed. We must disable all
        ...
        if(deviceObj->inUse) 
        {
            ...
            if((USB_HOST_PNP_IDENTIFIER(deviceInterfaceHandle)) == (USB_HOST_PNP_IDENTIFIER(deviceObj->deviceIdentifier)))
            {
                ...
                if ( endpointDescriptor != NULL )
                {
                     ...
                     * in an interrupt context. We dont check for interrupt safety here,
                    ...
                    {
                        /* If the mutex lock failed for unknown reason, we dont do anything
                    ...
                    if(result != USB_HOST_PIPE_HANDLE_INVALID)
                    {
                        ...
                        pipeObj->pipeHandle = deviceObj->hcdInterface->hostPipeSetup( deviceObj->hcdHandle,
                                ...
                                deviceObj->hubHandle,       /* Hub address and port */
        ...
        else
        {
            ...
            else
            {
                ...
                *requestHandle = USB_HOST_REQUEST_HANDLE_INVALID;
                ...
                /* Get a mutex lock as the the control transfer object is a global resource */
...
/* Function:
  ...
  Description:
    ...
    will point to the standard interace association descriptor.  The search
...
/* Function:
  ...
  Description:
    ...
    will point to the standard interace descriptor and class specific interface
...
/* Function:
  ...
  Description:
    ...
    device.  The specified inteface should have been claimed. The completion of
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        /* Map the string type to string index */
                        switch(stringType)
                        {
                        ...
                        else
                        {
                            ...
                            *requestHandle = USB_HOST_REQUEST_HANDLE_INVALID;
                            ...
                            /* Ger a mutex lock as the control transfer object is a global
                             * resource */
