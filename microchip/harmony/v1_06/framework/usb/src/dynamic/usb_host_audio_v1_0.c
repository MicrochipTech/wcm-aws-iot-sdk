...
*/
 ...
 )
 {
    ...
    /* This function is being called because an Audio V1 device was attached 
    ...
    switch (interfaceDescriptor->bInterfaceSubClass)
    {
        case USB_AUDIO_AUDIOCONTROL:
            /* Check if there is a free Audio V1 object.*/
        ...
        case USB_AUDIO_AUDIOSTREAMING:
            ...
            do
            {
                ...
                if (interfaceDescriptor != NULL)
                {
                    ...
                    audioStream = (USB_HOST_AUDIO_STREAM_SETTING *)&audioInstanceInfo->streamInf[audioInstanceInfo->countASInterfaces].audioStream[interfaceQuery.bAlternateSetting];
                    ...
                    { 
                        ...
                        if (endpointDescriptor != NULL)
                        {
                            ...
                            audioStream->state = USB_HOST_AUDIO_V1_STREAM_STATE_READY; 
                        }
                        else
                        {
                            USB_HOST_DeviceEndpointQueryContextClear(&endpointQuery);
                            ...
                            else
                            {
                                /* Error has occurred */
                                error = true; 
                                audioStream->state = USB_HOST_AUDIO_V1_STREAM_STATE_ERROR; 
                            }
                        }
                    }
                } 
...
)
{
    ...
    USB_HOST_AUDIO_V1_STREAM_EVENT_INTERFACE_SET_COMPLETE_DATA audioStrmIntrfcSetData; 
    ...
    switch(event)
    {
        case USB_HOST_DEVICE_INTERFACE_EVENT_TRANSFER_COMPLETE:
            ...
            else if (versionFlag == USB_HOST_AUDIO_V1_API_VERSION_FLAG_V1_0_DEPRECIATED)
            {
                ...
                audioStrm = &asInterface->audioStream[alternateSetting];
        ...
        case USB_HOST_DEVICE_INTERFACE_EVENT_SET_INTERFACE_COMPLETE:
            ...
            /* Get the requested alternate setting, we saved this at the time of
               request submit */
            alternateSetting = (uint8_t)context;
            
            /* Get the event */
            audioStreamEventLocal = (uint8_t) (context>>8); 
            
            /* Get pointer to Audio Stream  */
            audioStrm = &asInterface->audioStream[alternateSetting];
               
            DataIntrfcSetEvent = (USB_HOST_DEVICE_INTERFACE_EVENT_SET_INTERFACE_COMPLETE_DATA*)(eventData);
            audioStrmIntrfcSetData.requestHandle = DataIntrfcSetEvent->requestHandle; 
            audioStrmIntrfcSetData.requestStatus = DataIntrfcSetEvent->result; 
                    
            if (DataIntrfcSetEvent->result == USB_HOST_RESULT_SUCCESS)
            {
                ...
                if (alternateSetting)
                {
                    /* Alternate Setting is a Non Zero Value. That means we have 
                       ...
                       Alternate Setting. */
                    if (audioStrm->nEndpoints)
                    {
                        /* Open pipe for Audio Data Endpoint*/
                        if (asInterface->isIsoDataPipeSet == false)
                        {
                            endpointAddress = audioStrm->isoDataEndpointDesc.bEndpointAddress; 
            ...
            if (audioStreamEventLocal == USB_HOST_AUDIO_V1_API_VERSION_FLAG_STREAM_INTERFACE_SET)
            {
                ...
                {
                    asInterface->streamEventHandler(strmHandle, audioStreamEvent, 
                                    &audioStrmIntrfcSetData, asInterface->context);
                }
            }
            else if (audioStreamEventLocal == USB_HOST_AUDIO_V1_API_VERSION_FLAG_STREAM_DISABLE)
            {
                ...
                {
                    audioStrm->streamEventHandler(strmHandle, audioStreamEvent, 
                                        &audioStrmIntrfcSetData, audioStrm->context);
                }
            }
...
// *****************************************************************************
/* Function:
    USB_HOST_AUDIO_V1_RESULT USB_HOST_AUDIO_V1_ControlRequest
    (
...
{
    USB_HOST_AUDIO_V1_RESULT result = USB_HOST_AUDIO_V1_RESULT_FAILURE; 
...
)
{
    ...
    *streamingInterfaceObjNext = (USB_HOST_AUDIO_V1_STREAMING_INTERFACE_OBJ)(audioInstanceInfo->index || aSIntrfcIdxNext>>8); 
...
)
{
    ...
    alternateSetting = audioInstanceInfo->streamInf[aSIntrfcIdx].audioStream[asIntrfcSettingIdx].interfaceAlternateSetting; 
    ...
    switch (hostResult)
    {
        case USB_HOST_RESULT_SUCCESS:
            ...
            break; 
        case USB_HOST_RESULT_DEVICE_UNKNOWN:
            result = USB_HOST_AUDIO_V1_RESULT_DEVICE_UNKNOWN; 
            break; 
        default:
...
)
{
    ...
    audioStream = &asInterface->audioStream[alternateSetting];
    if (audioStream == NULL)
    {
        return USB_HOST_AUDIO_V1_RESULT_HANDLE_INVALID;
    }
...
)
{
    ...
    result = USB_HOST_AUDIO_V1_ControlRequest
    (
        audioDeviceObj,
        requestHandle,
        (USB_SETUP_PACKET *)&setupPacket,
...
)
{
    ...
    return (USB_AUDIO_V1_FORMAT_TAG)audioInstanceInfo->streamInf[aSIntrfcIdx].audioStream[asIntrfcSettingIdx].bTerminalLink;
}
...
)
{
    ...
    return (USB_AUDIO_V1_FORMAT_TAG)audioInstanceInfo->streamInf[aSIntrfcIdx].audioStream[asIntrfcSettingIdx].wFormatTag; 
}
...
)
{
    ...
    if (audioInstanceInfo != NULL)
    {
        ...
        nChannels =  audioInstanceInfo->streamInf[aSIntrfcIdx].audioStream[asIntrfcSettingIdx].bNrChannels;
    }
    return nChannels; 
}
...
)
{
    ...
    return audioInstanceInfo->streamInf[aSIntrfcIdx].audioStream[asIntrfcSettingIdx].bSubframeSize; 
}
...
)
{
    ...
    if (audioInstanceInfo != NULL)
    {
        bitResolution = audioInstanceInfo->streamInf[aSIntrfcIdx].audioStream[asIntrfcSettingIdx].bBitResolution;
    }
...
)
{
    ...
    return audioInstanceInfo->streamInf[aSIntrfcIdx].audioStream[asIntrfcSettingIdx].bSamFreqType; 
}
...
)
{
    ...
    return audioInstanceInfo->streamInf[aSIntrfcIdx].audioStream[asIntrfcSettingIdx].tSamFreq; 
}
...
)
{
    ...
    dataEndpointDescriptor = &audioInstanceInfo->streamInf[aSIntrfcIdx].audioStream[asIntrfcSettingIdx].isoDataEndpointDesc; 
...
)
{
    ...
    audioStream =  &asInterface->audioStream[nextAlternateSettingIndex]; 
    /* Stream Object is made from 
       LSB = audio instance index 
       Second byte  = Stream Interface Index
...
{ 
    ...
    if (streamHandle != USB_HOST_AUDIO_V1_0_STREAM_HANDLE_INVALID)
    {
        ...
        audioInstanceInfo = &gUSBHostAudioInstance[audioInstanceIndex];
        asInterface = &audioInstanceInfo->streamInf[asIntrefaceIndex]; 
        audioStream =  &(asInterface->audioStream[alternateSettingIndex]);
