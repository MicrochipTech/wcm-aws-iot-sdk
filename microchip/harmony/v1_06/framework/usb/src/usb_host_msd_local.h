...
/************************************************
 * An MSD command request object that is used by
 * the MSD object
 ************************************************/
typedef struct
{
    /* True if the object is currently being used */
    bool inUse;

    /* Setup packet for this command request */
...
typedef enum
{
    ...
    /* Reset the MSD interface */
    USB_HOST_MSD_STATE_RESET_COMMAND,
            
    /* Waiting for Reset to complete */
    USB_HOST_MSD_STATE_WAIT_FOR_RESET,

    /* Clear halt on IN pipe */
    USB_HOST_MSD_STATE_IN_PIPE_HALT_CLEAR,

    /* Wait for the halt clear to complete */
    USB_HOST_MSD_STATE_WAIT_IN_PIPE_HALT_CLEAR_COMPLETE,

    /* Waiting for post pipe halt clear CSW */
    USB_HOST_MSD_STATE_WAIT_FOR_POST_PIPE_HALT_CLEAR_CSW,
            
    /* Clear halt on OUT pipe */
    USB_HOST_MSD_STATE_OUT_PIPE_HALT_CLEAR,
            
    /* Wait for the halt clear to complete */
    USB_HOST_MSD_STATE_WAIT_OUT_PIPE_HALT_CLEAR_COMPLETE,
