...
)
{
    ...
    *transferHandle = USB_DEVICE_AUDIO_TRANSFER_HANDLE_INVALID;
    ...
    if ((direction == USB_DEVICE_AUDIO_WRITE) && &(thisAudioInstance->infCollection.streamInf[streamInfIndex].alterntSetting[activeAlternateSetting].isoSyncEp))
    {
        tempEndpointInstance = &(thisAudioInstance->infCollection.streamInf[streamInfIndex].alterntSetting[activeAlternateSetting].isoSyncEp);
    }
