...
/* Function:
  ...
  Description:
    ...
    will cause the device to generate resume signalling for 10 milliseconds. The
    resume signalling will stop after 10 milliseconds. The application can use
    ...
    manually start, maintain duration and then stop the resume signalling.
...
void USB_DEVICE_RemoteWakeupStartTimed ( USB_DEVICE_HANDLE usbDeviceHandle )
{
    USB_DEVICE_OBJ * usbDeviceThisHandle;

    /* Validate the device handle */
    usbDeviceThisHandle = _USB_DEVICE_ClientHandleValidate(usbDeviceHandle);
    if(NULL == usbDeviceThisHandle)
    {
    ...
    DRV_USB_DEVICE_RemoteWakeup_Start(usbDeviceThisHandle->usbCDHandle);
    
    /* Generate 10 Milli Seconds Delay */
    SYS_TMR_CallbackSingle(10,(uintptr_t )usbDeviceThisHandle,
...
/* Function:
  ...
  Summary:
    This function is always invoked by the timer system service when a specifed
    delay is completed.

  Description:
    This is a callback fucntion invoked by  Timer system service once a delay
...
void _USB_DEVICE_RemotewakeupTimerCallback(uintptr_t context)
{
    /* Retrive USB Device Layer Handle */
    USB_DEVICE_OBJ * usbDeviceThisHandle = (USB_DEVICE_OBJ*)context;
  
    /* Call USBCD remote wakeup stop fucntion. */
    DRV_USB_DEVICE_RemoteWakeup_Stop(usbDeviceThisHandle->usbCDHandle);
