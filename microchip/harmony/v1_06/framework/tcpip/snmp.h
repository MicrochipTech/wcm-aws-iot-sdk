...
/* This Macro is used for both SNMP SET and GET Variable processing to indicate the start of SNMP variable processing.
For multi byte data request, the first byte will be always SNMP_START_OF_VAR.  */
#define SNMP_START_OF_VAR       (0)

/* This Macro is used for both SNMP SET and GET Variable processing to indicate the end of SNMP variable processing.
...
TCPIP_SNMP_ExactIndexGet and TCPIP_SNMP_NextIndexGet returns a valid index as per SNMP_INDEX_INVALID. */
...
/*
  ...
  Description:
    Only dynamic and AgentID variables can contain ID.  MIB2BIB utility enforces this
	rules when BIB was generated.All the dynamic IDs are are listed in mib.h.
	Maximum value of a Dynamic ID is 1023.
...
/*
  ...
  Remarks:
    SequenceVar - This command is part of Microchip MIB script declares a previously defined OID variable 
	as a sequence variable and assigns an index to it. A sequence variable can contain an array of values 
	and any instance of its values can be referenced by an index. More than one sequence variable may 
	share a single index, creating multi dimensional arrays. 
...
/*
  ...
  Description:
    SNMP agent supports both IPv4 and IPv6  trap address type and is able to transmit traps 
	to both IPv4 and IPv6 Host receiver address.
...
// *****************************************************************************
// *****************************************************************************
// Section: SNMP GET and SET request routines
// *****************************************************************************
// *****************************************************************************
...
/*
  ...
  Precondition:
    SNMP module should be initialized.
...
/*
  ...
  Summary:
    This routine Set MIB variable with the requested value.
...
/*
  ...
  Description:
	...
	/- If SNMP Sequence MIB variable processing is performed with index or instance value
	then it is the responsibility of the agent to send the next available index for that 
	requested variable. This is a only one iteration process. Manager won't send any further 
	request for the received index value.
	 
	/- If SNMP request for a sequence variable starts with only OID without any instance, that is 
	a complete table is requested without any instance,
	Then It is the responsibility of the agent to send the first available index of the table.
	Then Manager will continue the request with the transferred instance until it receives
...
/*
  ...
  Returns:
    - true - if community string is collected
	- false - error
...
/*
  ...
  Summary:
    Set the writeCommunity String with SNMP index.
...
/*
  ...
  Summary:
    Set the readCommunity String with SNMP index.
...
/*
  ...
  Description:
    ...
    MIB IDs here as per SNMP version.
  ...
  Returns:
    - true    -   record ID exists .
    - false   -   otherwise.

  Remarks:
    This function is specific for Record ID validation and this can be used to 
	restrict OID string also.    	
...
/*
  ...
  Summary:
    Set the specific trap, generic trap and trap ID.

  Description:
    SNMP user need to call this function before transmitting any trap.This function will set 
	vendor specific trap , generic trap and default trap ID value.SNMPv2 trap will use this 
	trap ID while sending a specific trap.  
  ...
  Remarks:
    Trap ID is the NOTIFICATION-TYPE of the ASN.1 MIB format. From Trap ID , SNMP agent will 
	able to get OID string and OID string will be used while preparing TRAPv2 second varbind .
 */
void TCPIP_SNMP_TrapSpecificNotificationSet(uint8_t specTrap,uint8_t genTrap, SNMP_ID trapID);
...
/*
  ...
  Summary:
    Get the status of trap send flag.
  ...
  Parameters:
    trapNotify - true when more than one varbind are waiting to be part of PDU
...
/*
  ...
  Summary:
    Get the Socket ID for SNMP Server socket.
...
/*
  ...
  Summary:
    Set the Socket ID for SNMP Server socket.
...
/*
  ...
  Summary:
    Set the status of trap send flag.
...
/*
  ...
  Summary:
    Set the status of authentication trap flag.

  Description:
    This function is used to set the authentication trap send flag and this is used only when
    user is trying to send authetication failure trap. Ex- sending a trap if community do not
  ...
  Parameters:
    sendTrap - true if an auth trap need to be send to the manager
            - false
...
/*
  ...
  Summary:
    Get the status of authentication trap flag.

  Description:
    This function is used to Get the authentication trap send flag and this is used only when
    user is trying to send authetication failure trap. Ex- sending a trap if community do not
    match to the global community table.
...
/*
  ...
  Summary:
    Get the agent MIB ID for SNP notification.
...
/*
  ...
  Summary:
    Set the agent MIB ID for SNP notification.
...
/*
  ...
  Summary:
    Set the TRAP interface for SNMP notification.
...
/*
  ...
  Remarks:
     ...
     event occurred and should update corresponding OID and notification
...
/*
  ...
  Summary:
    Get SNMP Trap UDP client open socket time out.
...
/*
  ...
  Remarks:
    This API is used by the customer function.
*/
bool TCPIP_SNMP_TRAPTypeGet(void);
...
/*
  ...
  Summary:
    Get SNMP Trap type for version v3.
...
/*
  ...
  Summary:
    Get the status of trap interface.
...
/*
  ...
  Description:
    Performs SNMP module tasks in the TCP/IP stack.

  Precondition:
    SNMP module should have been initialized
