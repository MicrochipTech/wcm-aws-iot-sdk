...
uint16_t TCPIP_Helper_PacketCopy(TCPIP_MAC_PACKET* pPkt, uint8_t* pDest, uint8_t** pStartAdd, uint16_t len)
{
    TCPIP_MAC_DATA_SEGMENT* pSeg;
    ...
    pSeg = TCPIP_PKT_DataSegmentGet(pPkt, pSrcBuff, true);
...
bool  TCPIP_Helper_ProtectedSingleListInitialize(PROTECTED_SINGLE_LIST* pL)
{
    ...
    if (OSAL_SEM_Create(&pL->semaphore, OSAL_SEM_TYPE_BINARY, 1, 1) != OSAL_RESULT_TRUE)
    {
        // SYS_DEBUG
        pL->semaphore = 0;
        return false;
