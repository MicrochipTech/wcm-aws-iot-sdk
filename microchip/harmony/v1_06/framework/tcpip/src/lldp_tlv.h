...
// function to create a TLV
// returns the size needed for TLV or -1 if TLV cannot be created
// pBuff could be 0, if only size needed;
// if pBuff is not zero, it will write the LV at pBuff
// Note: it assume there's enough space in the buffer!!!
typedef size_t (*createLLDPFuncPtr)(int subType, uint8_t* pBuff, size_t buffSize);

typedef struct
{
    int                 subType;
    createLLDPFuncPtr   createFnc;
}createTLV_t;


// structure that points to a TLV createTLV_t table 
typedef struct
{
    const createTLV_t*  tlvTbl;
    int                 tblEntries;
}createTLV_tbl;
...
// pBuff could be 0 if just info sixe needed
size_t mibConstructInfoLLDPDU(uint8_t* pBuff, size_t buffSize);


void tlvInit(void);

// standard TLVs processing functions
//
