...
static const TCPIP_IPV6_MODULE_CONFIG  ipv6DefaultConfigData =
{   
...
bool TCPIP_IPV6_Initialize(const TCPIP_STACK_MODULE_CTRL* const pStackInit, const TCPIP_IPV6_MODULE_CONFIG* pIpv6Init)
{
    ...
    {   // stack going up
        if(pIpv6Init == NULL)
            pIpv6Init =  &ipv6DefaultConfigData;

        // stack initialization
        if(ipv6ModuleInitCount == 0)
        {   // 1st time we run
            TCPIP_Notification_Initialize(&ipv6RegisteredUsers);
            TCPIP_Helper_ProtectedSingleListInitialize (&mcastQueue);
            ipv6MemH = pStackInit->memH;
            TCPIP_Helper_ProtectedSingleListInitialize (&ipv6QueuedPackets);
            ...
            if (ipv6TaskHandle == 0)
            {
                ipv6TaskHandle =_TCPIPStackSignalHandlerRegister(TCPIP_THIS_MODULE_ID, TCPIP_IPV6_Task, TCPIP_IPV6_INIT_TASK_PROCESS_RATE);
            }
...
static void TCPIP_IPV6_UlaTask (void)
{
    ...
    switch(ulaState)
    {
        ...
        case TCPIP_IPV6_ULA_TSTAMP_GET:
            ...
            // calculate the SHA1
            SHA1Initialize(&shaSum);
            SHA1AddData(&shaSum, ulaTStamp.b, sizeof(ulaTStamp));
            SHA1Calculate(&shaSum, shaDigest);
