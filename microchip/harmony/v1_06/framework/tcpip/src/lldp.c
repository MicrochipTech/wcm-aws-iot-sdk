...
static const TCPIP_LLDP_MODULE_CONFIG lldpConfigDefault = 
{
};
...
static TCPIP_MAC_PKT_ACK_RES rxProcessFrame(TCPIP_MAC_PACKET* pRxPkt)
{
    ...
    while(tlvRes == TLV_PROCESS_CONTINUE)
    {
        ...
        {
            tlvRes = lldpProcessTlv(pTlv, lastType);
        }
...
static bool txFrame(void)
{
    ...
    lldpFrameSize = mibConstructInfoLLDPDU(0, 0);
    ...
    mibConstructInfoLLDPDU(pTxPkt->pNetLayer, lldpFrameSize);
...
{
    orgProcessFlags orgFlags;
    
    TCPIP_LLDP_OrgProcessFlagsGet(&orgFlags);
    
    return orgFlags.poeEnabledPower != 0;
