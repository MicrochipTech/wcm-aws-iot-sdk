...
static void _TelnetSocketRxSignalHandler(TCP_SOCKET hTCP, TCPIP_NET_HANDLE hNet, TCPIP_TCP_SIGNAL_TYPE sigType, const void* param);
...
bool TCPIP_TELNET_Initialize(const TCPIP_STACK_MODULE_CTRL* const stackCtrl, const TCPIP_TELNET_MODULE_CONFIG* pTelConfig)
{
    ...
    {   // first time we're run
        ...
        for(tIx = 0; tIx < sizeof(telnetDcpt)/sizeof(*telnetDcpt); tIx++, pDcpt++)
        {
            pDcpt->telnetSkt = TCPIP_TCP_ServerOpen(IP_ADDRESS_TYPE_ANY, TCPIP_TELNET_SERVER_PORT, 0);
            sigHandle = TCPIP_TCP_SignalHandlerRegister(pDcpt->telnetSkt, TCPIP_TCP_SIGNAL_RX_DATA, _TelnetSocketRxSignalHandler, 0);
