...
static const TCPIP_TCP_MODULE_CONFIG  tcpConfigDefault = 
{
...
static OSAL_SEM_HANDLE_TYPE tcpSemaphore = 0;
static OSAL_RESULT tcpSemaphoreEnabled = OSAL_RESULT_FALSE;
...
bool TCPIP_TCP_Initialize(const TCPIP_STACK_MODULE_CTRL* const stackInit, const TCPIP_TCP_MODULE_CONFIG* pTcpInit)
{
    ...
    OSAL_CRITSECT_DATA_TYPE status = OSAL_CRIT_Enter(OSAL_CRIT_TYPE_LOW);  // Full System Lock
    if(tcpInitCount != 0)
    {   // initialize just once
        tcpInitCount++;
        OSAL_CRIT_Leave(OSAL_CRIT_TYPE_LOW, status);  // Full System Lock
        return true;
    }
    ...
    nSockets = pTcpInit->nSockets?pTcpInit->nSockets:TCPIP_TCP_MAX_SOCKETS;
    ...
    tcpSemaphoreEnabled = OSAL_SEM_Create(&tcpSemaphore, OSAL_SEM_TYPE_BINARY, 1, 1);
    ...
    {   // we're up and running
        ...
        {   // stack shut down
            ...
            {   // all closed
                // release resources
                _TcpCleanup();
                if (tcpSemaphoreEnabled == OSAL_RESULT_TRUE)
                {
                    if (OSAL_SEM_Delete(&tcpSemaphore) != OSAL_RESULT_TRUE)
                    {
                        tcpSemaphoreEnabled = OSAL_RESULT_FALSE;
                    }
                }
...
uint16_t TCPIP_TCP_ArrayPut(TCP_SOCKET hTCP, const uint8_t* data, uint16_t len)
{
    ...
    if (pSkt->tlsSecured)
    {
        return NET_TLS_Write(pSkt->tlsHandle, data, len);
    }
#endif
    return TCPIP_TCP_ArrayPutRaw(hTCP, data, len);
}
...
uint16_t TCPIP_TCP_ArrayGet(TCP_SOCKET hTCP, uint8_t* buffer, uint16_t len)
{
    ...
    if (pSkt->tlsSecured)
    {
        return NET_TLS_Read(pSkt->tlsHandle, buffer, len);
    }
#endif
    return TCPIP_TCP_ArrayGetRaw(hTCP, buffer, len);
}
...
uint16_t TCPIP_TCP_ArrayPeek(TCP_SOCKET hTCP, uint8_t *vBuffer, uint16_t wLen, uint16_t wStart)
{
    ...
    if (pSkt->tlsSecured)
    {
        return NET_TLS_Peek(pSkt->tlsHandle, vBuffer, wLen);
    }
#endif
...
uint16_t TCPIP_TCP_ArrayFind(TCP_SOCKET hTCP, const uint8_t* cFindArray, uint16_t wLen, uint16_t wStart, uint16_t wSearchLen, bool bTextCompare)
{
    ...
    if (pSkt->tlsSecured)
    {
      wDataLen = TCPIP_TCP_ArrayPeek(hTCP, tlsBuffer, sizeof(tlsBuffer), 0);
    }
    else
#endif
    {
    ...
    if (pSkt->tlsSecured)
    {
        ptrLocation = &tlsBuffer[wStart];
        if (ptrLocation > &tlsBuffer[sizeof(tlsBuffer)-1])
        {
            ptrLocation -= &tlsBuffer[sizeof(tlsBuffer)] -  tlsBuffer + 1;
        }
        wBytesUntilWrap = &tlsBuffer[sizeof(tlsBuffer)] - ptrLocation + 1;
    }
    else
#endif
    {
...
static void _TcpCloseSocket(TCB_STUB* pSkt, TCPIP_TCP_SIGNAL_TYPE tcpEvent)
{
    ...
    if (pSkt->tlsSecured)
    {
        NET_TLS_ConnectionClose(pSkt->tlsHandle);
        pSkt->tlsSecured = false;
    }
#endif
    ...
    if(len)
    {
        ...
        if((int16_t)wMissingBytes <= 0)
        {
            ...
            if(pSkt->rxHead + len > pSkt->rxEnd)
            {
                ...
                nCopiedBytes = TCPIP_Helper_PacketCopy(pRxPkt, pSkt->rxHead, &pSegSrc, wTemp);
                nCopiedBytes += TCPIP_Helper_PacketCopy(pRxPkt, pSkt->rxStart, &pSegSrc, len - wTemp);
                newRxHead = pSkt->rxStart + (len - wTemp);
            }
            else
            {
                nCopiedBytes = TCPIP_Helper_PacketCopy(pRxPkt, pSkt->rxHead, &pSegSrc, len);
        ...
        {   // wMissingBytes  > 0: this packet contains ahead data
            ...
            if(pSkt->rxHead + wMissingBytes + len > pSkt->rxEnd)
            {
                ...
                if((int32_t)wTemp >= 0)
                {
                    nCopiedBytes = TCPIP_Helper_PacketCopy(pRxPkt, pSkt->rxHead + wMissingBytes, &pSegSrc, wTemp);
                    nCopiedBytes += TCPIP_Helper_PacketCopy(pRxPkt, pSkt->rxStart, &pSegSrc, len - wTemp);
                }
                else
                {
                    nCopiedBytes = TCPIP_Helper_PacketCopy(pRxPkt, pSkt->rxHead + wMissingBytes - (pSkt->rxEnd - pSkt->rxStart + 1), &pSegSrc, len);
                }
            }
            else
            {
                nCopiedBytes = TCPIP_Helper_PacketCopy(pRxPkt, pSkt->rxHead + wMissingBytes, &pSegSrc, len);
            }
...
{
#if defined(NET_TLS_USE_CLIENT) && defined(NET_TLS_USE_STREAM)
    TCB_STUB* pSkt = _TcpSocketChk(hTCP);
    if (pSkt == 0)
    {
        return false;
    }
    pSkt->tlsHandle = NET_TLS_StreamClientStart(hTCP, host, certIndex);
    if (pSkt->tlsHandle == NET_TLS_INVALID_HANDLE)
    {
        return false;
    }
    pSkt->tlsSecured = true;
    return true;
#else
    return false;
#endif
...
/*****************************************************************************
  ...
  Parameters:
    ...
    certIndex   - Passed through to the TLS layer's serverCreate function.  It
                  ...
                  server certificate, or ic can be ignored
...
{
#if defined(NET_TLS_USE_SERVER) && defined(NET_TLS_USE_STREAM)
    TCB_STUB* pSkt = _TcpSocketChk(hTCP);
    if (pSkt == 0)
    {
        return false;
    }
    pSkt->tlsHandle = NET_TLS_StreamServerStart(hTCP, certIndex);
    if (pSkt->tlsHandle == NET_TLS_INVALID_HANDLE)
    {
        return false;
    }
...
#if defined(NET_TLS_USE_STREAM)
    TCB_STUB* pSkt = _TcpSocketChk(hTCP);
    if (pSkt == 0 || !pSkt->tlsSecured)
    {
        return false;
    }
    return NET_TLS_StillHandshaking(pSkt->tlsHandle);
...
bool  TCPIP_TLSTCP_SocketIsSecuredByTLS(TCP_SOCKET hTCP)
{
    ...
    return NET_TLS_IsSecuredByTLS(pSkt->tlsHandle);
