...
void _cfgBsdSocket(const struct BSDSocket * socketInfo)
{
    ...
    if (socketInfo->SocketType == SOCK_DGRAM)
    {
        ...
        {
            TCPIP_UDP_OptionsSet(socketInfo->SocketID, UDP_OPTION_TX_BUFF, (void*)socketInfo->sndBufSize);
        }
        if(socketInfo->udpBcastEnabled != 0)
        {
            TCPIP_UDP_BcastIPV4AddressSet(socketInfo->SocketID, UDP_BCAST_NETWORK_LIMITED, 0);
        }
...
int bind( SOCKET s, const struct sockaddr* name, int namelen )
{
    ...
    if (socket->SocketType == SOCK_DGRAM)
    {
        ...
        {
#endif
            if (!TCPIP_UDP_Bind(socket->SocketID, IP_ADDRESS_TYPE_IPV4, lPort, (((struct sockaddr_in *) name)->sin_addr.S_un.S_addr == IP_ADDR_ANY) ? 0 : (IP_MULTI_ADDRESS*) & lAddr))
            {
                errno = EINVAL;
                return SOCKET_ERROR;
            }
        ...
        {
            if (!TCPIP_UDP_Bind(socket->SocketID, IP_ADDRESS_TYPE_IPV6, lPort, (IP_MULTI_ADDRESS*) & lAddr6))
            {
                errno = EINVAL;
                return SOCKET_ERROR;
            }
...
int recv( SOCKET s, char* buf, int len, int flags )
{
	...
	if(socket->SocketType == SOCK_STREAM) //TCP
	{
		...
		nBytes = TCPIP_TCP_GetIsReady(socket->SocketID);
        if(nBytes && buf && len)
	...
	else if(socket->SocketType == SOCK_DGRAM) //UDP
	{
        ...
        nBytes = TCPIP_UDP_GetIsReady(socket->SocketID);
        if(nBytes && buf && len)
        {   // copy available data to user buffer
            ...
            if (nBytes <= len)
            {
                ...
                TCPIP_UDP_Discard(socket->SocketID);
            }
        }
...
static bool HandlePossibleTCPDisconnection(SOCKET s)
{
	...
	if(bSocketWasReset)
	{
		TCPIP_TCP_Abort(socket->SocketID, false);
...
struct hostent * gethostbyname(char *name)
{
    ...
    if (sHaveDnsToken == 0)
    {
        ...
        if (dRes == DNS_RES_NAME_IS_IPADDRESS)
        {
                ...
                struct addrinfo **res)
{
    ...
    switch (sgaistate)
    {
        ...
        case TCPIP_BERKELEY_GAI_START_IPV4:
        {
            ...
            {
                TCPIP_DNS_RESULT result = TCPIP_DNS_Resolve(node, DNS_TYPE_A);
                switch (result)
                {
                    ...
                    case DNS_RES_NAME_IS_IPADDRESS:
                    {
                        struct addrinfo * ptr;
                        IPV4_ADDR tmp;
        ...
        {
            IPV4_ADDR tmp;
            TCPIP_DNS_RESULT result = TCPIP_DNS_IsResolved(node, &tmp);
            switch (result)
            {
                case DNS_RES_NO_SERVICE:
                case DNS_RES_SERVER_TMO:
                ...
                default:
                    SYS_ASSERT(false, "TCPIP_DNS_IsResolved is passing back a new error");
                    break;
            }
        }
#endif
        ...
        case TCPIP_BERKELEY_GAI_START_IPV6:
        {
            ...
            {
                TCPIP_DNS_RESULT result = TCPIP_DNS_Resolve(node, DNS_TYPE_AAAA);
                switch (result)
                {
                    case DNS_RES_NO_SERVICE:
                    {
        ...
        {
            IPV6_ADDR tmp;
            TCPIP_DNS_RESULT res = TCPIP_DNS_IsResolved(node, &tmp);
            switch (res)
            {
                case DNS_RES_NO_SERVICE:
                case DNS_RES_SERVER_TMO:
    ...
    case TCPIP_BERKELEY_GAI_FINISHED:
    {
        *res = NULL;
        ...
        int numDNS;
#ifdef TCPIP_STACK_USE_IPV4
        numDNS = TCPIP_DNS_GetNumberOfIPAddresses(node, IP_ADDRESS_TYPE_IPV4);
        for (counter = 0; counter < numDNS; counter++)
        {
            ...
            ((struct sockaddr_in*)(ptr->ai_addr))->sin_family = AF_INET;
            IPV4_ADDR tmp;
            TCPIP_DNS_GetIPv4Address(node, counter, &tmp);
            ((SOCKADDR_IN*)(ptr->ai_addr))->sin_addr.S_un.S_addr = tmp.Val;
        }
#endif
#ifdef TCPIP_STACK_USE_IPV6
        numDNS = TCPIP_DNS_GetNumberOfIPAddresses(node, IP_ADDRESS_TYPE_IPV6);
        for (counter = 0; counter < numDNS; counter++)
        {
            if (ptr == NULL)
            {
            ...
            IPV6_ADDR tmp;
            TCPIP_DNS_GetIPv6Address(node, counter, &tmp);
            ((SOCKADDR_IN6*)(ptr->ai_addr))->sin6_addr.in6_u.u6_addr32[0] = tmp.d[0];
            ((SOCKADDR_IN6*)(ptr->ai_addr))->sin6_addr.in6_u.u6_addr32[1] = tmp.d[1];
            ((SOCKADDR_IN6*)(ptr->ai_addr))->sin6_addr.in6_u.u6_addr32[2] = tmp.d[2];
            ((SOCKADDR_IN6*)(ptr->ai_addr))->sin6_addr.in6_u.u6_addr32[3] = tmp.d[3];
