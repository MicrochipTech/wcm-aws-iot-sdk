...
static const TCPIP_HTTP_MODULE_CONFIG httpConfigDefault = 
{
              ...
              const TCPIP_HTTP_MODULE_CONFIG* httpInitData)
{
    ...
    {   // first time we're run
...
#if !defined(NET_TLS_USE_STREAM) || !defined(NET_TLS_USE_SERVER)
        httpsConnNo = 0;
#else
        httpsConnNo = httpInitData->nTlsConnections;
#endif
        ...
        if(httpConnNo)
        {
...
#else
            httpConnCtrl = (HTTP_CONN*)TCPIP_HEAP_Calloc(stackCtrl->memH, (httpConnNo + httpsConnNo), sizeof(*httpConnCtrl));
            httpConnData = (uint8_t*)TCPIP_HEAP_Malloc(stackCtrl->memH, (httpConnNo + httpsConnNo) * httpInitData->dataLen);
#endif
            ...
            for(conn = 0; conn < httpConnNo + httpsConnNo ; conn++)
            {
                pHttpCon->sm = SM_HTTP_IDLE;

                if (conn >= httpConnNo)
                {
                    pHttpCon->tlsSocket = true;
                    pHttpCon->socket = TCPIP_TCP_ServerOpen(IP_ADDRESS_TYPE_ANY, TCPIP_HTTP_SERVER_SECURE_PORT, 0);
                    if (httpInitData->tlsSktTxBuffSize != 0)
                    {
                        void* tcpBuffSize = (void*)(unsigned int)httpInitData->tlsSktRxBuffSize;
                        TCPIP_TCP_OptionsSet(pHttpCon->socket, TCP_OPTION_TX_BUFF, tcpBuffSize);
                    }
                    if (httpInitData->tlsSktRxBuffSize != 0)
                    {
                        void* tcpBuffSize = (void*)(unsigned int)httpInitData->tlsSktRxBuffSize;
                ...
                TCPIP_TCP_SignalHandlerRegister(pHttpCon->socket, TCPIP_TCP_SIGNAL_RX_DATA, _HTTPSocketRxSignalHandler, 0);
...
static void TCPIP_HTTP_Process(void)
{
    ...
    pHttpCon = httpConnCtrl + 0;
    for(conn = 0; conn < httpConnNo + httpsConnNo; conn++, pHttpCon++)
    {
        if(pHttpCon->socket == INVALID_SOCKET)
            continue;
        ...
        if(TCPIP_TCP_WasReset(pHttpCon->socket))
        {
            ...
            if(((httpConfigFlags & HTTP_MODULE_FLAG_ADJUST_SKT_FIFOS) != 0) && !(pHttpCon->tlsSocket))
            {
...
static void TCPIP_HTTP_ProcessConnection(HTTP_CONN* pHttpCon)
{
    ...
    do
    {
        ...
        switch(pHttpCon->sm)
        {
...
#if defined(NET_TLS_USE_STREAM) && defined(NET_TLS_USE_SERVER)
                    if (pHttpCon->tlsSocket)
                    {
                        pHttpCon->sm = SM_HTTP_TLS_NEGO;
                        TCPIP_TLSTCP_ServerStart(pHttpCon->socket, 0);
                    }
#endif
                }
                else
            ...
            case SM_HTTP_TLS_NEGO:
            {
                // Check to see if we are actually in this state, if not fall
                // through to the next one.
                if (pHttpCon->sm == SM_HTTP_TLS_NEGO)
                {
                    if (TCPIP_TLSTCP_StillHandshaking(pHttpCon->socket))
                    {
                        break;
                    }
                    if (TCPIP_TLSTCP_SocketIsSecuredByTLS(pHttpCon->socket))
                    {
                        pHttpCon->httpTick = SYS_TMR_TickCountGet() + TCPIP_HTTP_TIMEOUT * SYS_TMR_TickCounterFrequencyGet();
                        pHttpCon->sm = SM_HTTP_PARSE_REQUEST;
                    }
                ...
                // Fall thorugh to the next state.
            }
            case SM_HTTP_PARSE_REQUEST:
                ...
                if(lenA > TCPIP_HTTP_MAX_DATA_LEN - TCPIP_HTTP_DEFAULT_LEN - 1)
                {
                    pHttpCon->httpStatus = HTTP_OVERFLOW;
                    pHttpCon->sm = SM_HTTP_SERVE_HEADERS;
                    isDone = false;
                ...
                if(lenA != 0u)
                {
                    ...
                    lenA--;
                    if(lenA >= TCPIP_HTTP_MAX_DATA_LEN - 2)
                    {
                        pHttpCon->httpStatus = HTTP_OVERFLOW;
                        pHttpCon->sm = SM_HTTP_SERVE_HEADERS;
                        isDone = false;
...
static void _HTTP_HeaderParseCookie(HTTP_CONN* pHttpCon)
{
    ...
    if(lenB >= (uint16_t)(pHttpCon->data + TCPIP_HTTP_MAX_DATA_LEN - pHttpCon->ptrData - 2))
    {// If not, overflow
        pHttpCon->httpStatus = HTTP_OVERFLOW;
        pHttpCon->sm = SM_HTTP_SERVE_HEADERS;
        return;
...
static HTTP_IO_RESULT TCPIP_HTTP_MPFSUpload(HTTP_CONN* pHttpCon)
{
    ...
    switch(pHttpCon->httpStatus)
    {
        ...
        case HTTP_MPFS_UP:
            ...
            {// Found it, so remove all data up to and including
                ...
                {   // Read as Ver 2.1
                    ...
                    {
                        pHttpCon->uploadBufferStart = (uint8_t*)TCPIP_STACK_MALLOC_FUNC(DRV_NVM_PAGE_SIZE);
                        SYS_FS_Unmount_Wrapper((const char *)&pHttpCon->data[1]);
                    }

                    if(pHttpCon->uploadBufferStart != 0)
                    {
                        pHttpCon->uploadBufferEnd = pHttpCon->uploadBufferStart + DRV_NVM_PAGE_SIZE;
        ...
        case HTTP_MPFS_OK:
            ...
            nSectors = ((pHttpCon->uploadBufferCurr - pHttpCon->uploadBufferStart) + DRV_NVM_ROW_SIZE - 1) / (DRV_NVM_ROW_SIZE);

            bufferHandle = SYS_FS_MEDIA_MANAGER_SectorWrite(MPFS_UPLOAD_DISK_NO, pHttpCon->uploadSectNo , pHttpCon->uploadBufferStart,
                        nSectors);
            if ( bufferHandle == SYS_FS_MEDIA_BLOCK_COMMAND_HANDLE_INVALID)
            {
                pHttpCon->httpStatus = HTTP_MPFS_ERROR;
            ...
            while(SYS_FS_MEDIA_MANAGER_CommandStatusGet(MPFS_UPLOAD_DISK_NO, bufferHandle) != SYS_FS_MEDIA_COMMAND_COMPLETED);
...
int TCPIP_HTTP_ActiveConnectionCountGet(void)
{
    ...
    pHttpCon = httpConnCtrl + 0;
    for(connIx = 0; connIx < httpConnNo + httpsConnNo; connIx++, pHttpCon++)
    {
