...
static const TCPIP_UDP_MODULE_CONFIG udpConfigDefault = 
{
...
static OSAL_SEM_HANDLE_TYPE userSem = 0;
...
static void*            _TxSktGetLockedV4Pkt(UDP_SOCKET_DCPT* pSkt, bool clear);
...
static bool _UDPv6TxAckFnc (void* pkt, bool success, const void * param)
{
    ...
    {  
        ...
        _UDPResetHeader(TCPIP_IPV6_UpperLayerHeaderPtrGet(pV6Pkt));
        pSkt->txWrite = pSkt->txStart;
...
#if defined(NET_TLS_USE_DATAGRAM)
    if (pSkt->extFlags.tlsSecured)
    {
        NET_TLS_ConnectionClose(pSkt->tlsHandle);
        pSkt->extFlags.tlsSecured = false;
    }
#endif
...
uint16_t TCPIP_UDP_ArrayPut(UDP_SOCKET s, const uint8_t *cData, uint16_t wDataLen)
{
    ...
    if (pSkt->extFlags.tlsSecured)
    {
        return NET_TLS_Write(pSkt->tlsHandle, cData, wDataLen);
    }
#endif
    return TCPIP_UDP_ArrayPutRaw(s, cData, wDataLen);
}

uint16_t TCPIP_UDP_ArrayPutRaw(UDP_SOCKET s, const uint8_t *cData, uint16_t wDataLen)
{
...
uint16_t TCPIP_UDP_ArrayGet(UDP_SOCKET s, uint8_t *cData, uint16_t reqBytes)
{
   ...
   UDP_SOCKET_DCPT* pSkt = _UDPSocketDcpt(s);
    ...
    if (pSkt->extFlags.tlsSecured)
    {
        return NET_TLS_Read(pSkt->tlsHandle, cData, reqBytes);
    }
#endif
    return TCPIP_UDP_ArrayGetRaw(s, cData, reqBytes);
}

uint16_t TCPIP_UDP_ArrayGetRaw(UDP_SOCKET s, uint8_t *cData, uint16_t reqBytes)
{
...
/*****************************************************************************
  Function:
    bool TCPIP_TLSUDP_ClientStart(TCP_SOCKET hTCP, uint8_t* host, uint8_t certIndex)
 ...
 */
bool  TCPIP_TLSUDP_ClientStart(TCP_SOCKET hUDP, uint8_t* host, uint8_t certIndex)
{
#if defined(NET_TLS_USE_CLIENT) && defined(NET_TLS_USE_DATAGRAM)
    UDP_SOCKET_DCPT *pSkt = _UDPSocketDcpt(hUDP);
    ...
    pSkt->tlsHandle = NET_TLS_DataGramClientStart(hUDP, host, certIndex);
    if (pSkt->tlsHandle == NET_TLS_INVALID_HANDLE)
    {
        return false;
    }
    pSkt->extFlags.tlsSecured = true;
    return true;
#else
    return false;
#endif
...
/*****************************************************************************
  Function:
    bool TCPIP_TLSUDP_ServerStart(TCP_SOCKET hTCP, uint8_t certIndex)
...
bool  TCPIP_TLSUDP_ServerStart(UDP_SOCKET hUDP, uint8_t certIndex)
{
    ...
    pSkt->tlsHandle = NET_TLS_DataGramServerStart(hUDP, certIndex);
    if (pSkt->tlsHandle == NET_TLS_INVALID_HANDLE)
    {
        return false;
    }
...
/*****************************************************************************
  Function:
    bool TCPIP_TLSUDP_StillHandshaking(TCP_SOCKET hTCP)

  Summary:
    Determines if an TLS session is still handshaking.

  Description:
    Call this function after calling TCPIP_TLSUDP_ClientStart until false is
...
bool  TCPIP_TLSUDP_StillHandshaking(UDP_SOCKET hUDP)
{
    ...
    UDP_SOCKET_DCPT *pSkt = _UDPSocketDcpt(hUDP);
    if (pSkt == 0 || !pSkt->extFlags.tlsSecured)
    {
        return false;
    }
