...
typedef struct
{
    ...
    uint16_t    nTlsConnections; //number of simultaneous HTTPS connections allowed
    ...
    uint16_t    tlsSktTxBuffSize;  // size of TLS TX buffer for the associated socket; leave 0 for default (min 512 bytes)
    uint16_t    tlsSktRxBuffSize;  // size of TLS RX buffer for the associated socket; leave 0 for default (min 512 bytes)
...
/*
  ...
  Description:
    Performs HTTP module tasks in the TCP/IP stack.

  Precondition:
    HTTP module should have been initialized
