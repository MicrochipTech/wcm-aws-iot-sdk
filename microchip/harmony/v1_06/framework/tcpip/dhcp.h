...
/* Enumeration: TCPIP_DHCP_STATUS
  ...
  Description:
    ...
    Used in getting info about the DHCP state machine. 
 */
typedef enum
{
...
/* Function:
  ...
  Summary:
    This function sets the DHCP client request and base timeout values

  Description:
    This function allows the run time adjustment of the DHCP timeout values.
    ...
    This is automatically incremented by the DHCP client using an exponential backoff
    algorithm.
    Recomended value is 2 <= dhcpBaseTmo <= 64 seconds.
  ...
  Parameters:
    ...
    initTmo     - initialization timeout to wait for a valid lease acquisition, in seconds
    dhcpBaseTmo - DHCP timeout value for DHCP requests, in seconds
...
/*****************************************************************************
  ...
  Description:
    Returns the current state and lease info of the DHCP client
  ...
  Returns:
    - true	- if the interface is enabled/exists and DHCP client service information filled in the supplied storage
    - false	- if the specified interface is not enabled/exists or doesn't have a DHCP client
 */
bool TCPIP_DHCP_InfoGet(TCPIP_NET_HANDLE hNet, TCPIP_DHCP_INFO* pDhcpInfo);
...
/* Function:
  ...
  Remarks:
    ...
    Simpy call TCPIP_DHCP_HostNameCallbackRegister with the nameCallback set to 0.
    ...
    (on either user supplied host name or NetBios name):
        ...
        - illegal charcters will be replaced by TCPIP_DHCP_HOST_REPLACE_CHAR 
        character (default is lower case 'x')

   If after processing the resulting string is null,
   then the DHCP host name option is skipped.
...
/*
  ...
  Description:
    Performs DHCP module tasks in the TCP/IP stack.
