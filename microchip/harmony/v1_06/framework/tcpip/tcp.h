...
{
    // TX related signals
    TCPIP_TCP_SIGNAL_TX_DONE         = 0x0001,   // A TCP packet was successfully transmitted on the interface.
                                                // This may indicate or not that new data can be sent with this socket.
                                                // Notes:
                                                //  - the TCP buffer space is controlled by the TCP state machine;
                                                //    the fact that a packet was transmitted, doesn't necessarily mean
                                                //    that new data can be written into the socket buffer: 
                                                //      - the packet may be an ACK only packet, retry packet, etc., and doesn't free
                                                //        any writing space in the socket buffer
                                                //      - new socket writing space may not be available because the other party has not 
                                                //        acknowledged the data it was sent so far
                                                //
                                                //    Therefore, the information carried by this signal has limited usage.
                                                //  
                                                //  - this notification is the result of an user action (explicit call to TCPIP_TCP_Flush)
                                                //    or as a result of the internal TCP state machine

    TCPIP_TCP_SIGNAL_TX_DATA_DONE    = 0x0002,   // A TCP data packet carrying valid payload was successfully transmitted on the interface.
                                                // This may indicate or not that new data can be sent with this socket.
                                                // Notes:
                                                //  The above notes for TCPIP_TCP_SIGNAL_TX_DONE still apply.
                                                //
                                               
    TCPIP_TCP_SIGNAL_TX_SPACE        = 0x0004,   // New TX space is available in the socket TX buffer
                                                // This event signals that the remote party has acknowledged some data and, as a result,
                                                // TX buffer space is available.
                                                //
...
/*
  ...
  Description:
    TCP module configuration/initialization
*/
//
typedef struct
{
...
/******************************************************************************
  ...
  Description:
    ...
    2. If the graceful option is not set, or the previous step could not send the FIN:
       A TCPIP_TCP_Abort() is called, sending a RST to the remote node. Communication is closed,
...
/*****************************************************************************
  ...
  Returns:
    ...
    Note that this operation can cause a TCP packet to be transmitted over the
    ...
    a) There is no more space available in the TCP buffer.
    b) The TCP_OPTION_THRES_FLUSH_ON is set and the TX buffer is at least half full
    c) The amount of data that can be sent is bigger than the remote host MSS
       or than half of the maximum advertised window size.
    d) Nagle algorithm is disabled and there is no unacknowledged data. 

    If none of this occurs and the socket user doesn't add data to the TX socket buffer,
...
/*****************************************************************************
  Function:
    uint16_t TCPIP_TCP_ArrayPutRaw(TCP_SOCKET hTCP, const uint8_t* data, uint16_t len)
 ...
 Remarks:
    This function provides the same functionality as TCPIP_TCP_ArrayPut, but does
    not check to see if the information needs to be encrypted before being sent out.
    This is mainly to provide a way of sending data through the TLS layer without
    causing a functional loop.
...
/******************************************************************************
  ...
  Description:
    Returns the number of bytes that are free in the socket TX buffer.
...
/*****************************************************************************
  ...
  Description:
    Reads an array of data bytes from a TCP socket's RX buffer/FIFO.  The data
...
/*****************************************************************************
  Function:
    uint16_t TCPIP_TCP_ArrayGetRaw(TCP_SOCKET hTCP, uint8_t* buffer, uint16_t len)
  ...
  Remarks:
    ...
    This function has the same functionality as TCPIP_TCP_ArrayGet, the only
    difference is that it doesn't check if the information needs decrypting first.
    This is mainly to provide a way of receiving data through the TLS layer without
    causing a functional loop.
 */
uint16_t  TCPIP_TCP_ArrayGetRaw(TCP_SOCKET hTCP, uint8_t* buffer, uint16_t count);
...
/*****************************************************************************
  ...
  Parameters:
    ...
    vFlags      - If TCP_ADJUST_TX_ONLY or TCP_ADJUST_RX_ONLY  are not set,
                    then the TX and RX buffers are evaluated together and any
...
/*****************************************************************************
  ...
  Remarks: An invalid hNet can be passed (0) so that the current
...
TCPIP_NET_HANDLE  TCPIP_TCP_SocketNetGet(TCP_SOCKET hTCP);
  ...
  Remarks:
    The returned handle could be NULL if
    the socket is invalid or
...
// *****************************************************************************
/* Function:
    TCPIP_TCP_SignalHandlerRegister(TCP_SOCKET s, TCPIP_TCP_SIGNAL_TYPE sigMask, TCPIP_TCP_SIGNAL_FUNCTION handler, const void* hParam)
...
/* Function:
  ...
  Description:
    Deregisters the TCP socket signal handler.
...
/*
  ...
  Description:
    Performs TCP module tasks in the TCP/IP stack.

  Precondition:
    TCP module should have been initialized
