...
#include "crypto/src/error.h"
    ...
    #include "crypto/src/wincrypt.h"
#else
    #if !defined(NO_DEV_RANDOM) && !defined(CYASSL_MDK_ARM) 
            #include "crypto/src/fcntl.h"
        #ifndef EBSNET
            #include "crypto/src/unistd.h"
        #endif
    #else
...
int GenerateSeed(OS_Seed* os, byte* output, word32 sz)
{
    ...
    #include "xc.h"
    #ifdef CYASSL_PIC32MZ_RNG
static unsigned int ReadCoreTimer()
{
    ...
    return(timer);
        ...
        int GenerateSeed(OS_Seed* os, byte* output, word32 sz)
        {
            ...
            // For PIC32MZ EC, the TRNG does not function, so seed the
            // PRNG from the core timer.
