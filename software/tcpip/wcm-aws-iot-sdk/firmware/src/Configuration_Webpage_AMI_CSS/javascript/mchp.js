...
function securityKeyParse(secCode, key) {
...
function switchNetwork(id) {
	...
	var ValidIpAddressRegex = new RegExp("^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)){3}$");
	...
	var ipOrDns;
	if(ValidIpAddressRegex.test(remoteServer) == true)
	{
		ipOrDns = "validIp";
	}
	else if(ValidHostnameRegex.test(remoteServer)== true)
	{
		ipOrDns = "validHostname";
	}
	else
	{
		alert('Please enter a fqdn (e.g. 123.123.123.123 or my.example.com.');
		return;
	}

	// Force infrastructure mode for this demo
	document.getElementById("wlan").value = "infra"; //(wlanCode == 1) ? "infra" : "adhoc";
	document.getElementById("ssid").value = name;
	document.getElementById("sec").value = getSecurity(parseInt(secCode));
	
	// If security code was 0, then we reset the pw value back to null in case user had entered a password but then chose
	// OPEN security.  This was throwing off the WCM state machine
	if(secCode == 0) {
		pw = "";
	}
	
	var rsSubstring = remoteServer; //.substring(8);
	document.getElementById("rs").value = rsSubstring;

	var key;
	if (parseInt(secCode)) {
		key = securityKeyParse(secCode, pw);
...
function joinNetwork() {
	...
	//Added 1.0
	var checkSecurity;
	checkSecurity = document.getElementById('sec1').value;
...
function userSelectNetwork() {
	...
	// adding new field for remote server ip address
	...
	if (otherNetworkExpanded) 
	{
   	// need to destroy table back to just button
				...
				row8.setAttribute('style', 'width:9em');
				var data10 = document.createElement("td");
				data10.setAttribute('colspan', '4');
				data10.appendChild(document.createTextNode("AWS Server Address (IPv4)"));
				row8.appendChild(data10);
				
				var row7 = document.createElement("tr");
				row7.setAttribute('style', 'width:9em');
				var data9 = document.createElement("td");
				data9.setAttribute('colspan', '1');
				var ipaddrInput1 = document.createElement("input");
				ipaddrInput1.setAttribute('type', 'text');
				ipaddrInput1.setAttribute('id', 'ipaddr1');
				ipaddrInput1.setAttribute('name', 'ipaddr1');
				ipaddrInput1.setAttribute('maxlength', '3');
				ipaddrInput1.setAttribute('size', '3');
				data9.appendChild(ipaddrInput1);
				data9.appendChild(document.createTextNode("."));
				var ipaddrInput2 = document.createElement("input");
				ipaddrInput2.setAttribute('type', 'text');
				ipaddrInput2.setAttribute('id', 'ipaddr2');
				ipaddrInput2.setAttribute('name', 'ipaddr2');
				ipaddrInput2.setAttribute('maxlength', '3');
				ipaddrInput2.setAttribute('size', '3');
				data9.appendChild(ipaddrInput2);
				data9.appendChild(document.createTextNode("."));
				var ipaddrInput3 = document.createElement("input");
				ipaddrInput3.setAttribute('type', 'text');
				ipaddrInput3.setAttribute('id', 'ipaddr3');
				ipaddrInput3.setAttribute('name', 'ipaddr3');
				ipaddrInput3.setAttribute('maxlength', '3');
				ipaddrInput3.setAttribute('size', '3');
				data9.appendChild(ipaddrInput3);
				data9.appendChild(document.createTextNode("."));
				var ipaddrInput4 = document.createElement("input");
				ipaddrInput4.setAttribute('type', 'text');
				ipaddrInput4.setAttribute('id', 'ipaddr4');
				ipaddrInput4.setAttribute('name', 'ipaddr4');
				ipaddrInput4.setAttribute('maxlength', '3');
				ipaddrInput4.setAttribute('size', '3');
				data9.appendChild(ipaddrInput4);
				row7.appendChild(data9);
				...
				tfoot.appendChild(row8);
				tfoot.appendChild(row7);
